...
def fit_pile_to_size(pile: list[str], all_played):
    if len(pile) < 20:
        shuffle(all_played)
        pile += all_played
        all_played = []
...
def parse_server_arguments(args: list[str]) -> tuple[tuple[str, int], int, list[bool]]:
    if len(args) < 4:
        log.critical("Please provide four space-separated arguments: the local ip address, the port"
...
def connect_clients(server_socket: socket):
    player_names: list[str] = []  # Names of players
    socks: list[socket] = []  # Socket information per player
    addresses = []  # Each player's address
    list_of_players = []  # Integer representations of players
    player_counter = 0
...
if __name__ == "__main__":
    ...
    local_address, num_players, modes = parse_server_arguments(sys.argv)
    ...
    # Deck initialisation
    d: list[Card] = Deck().deck
    pile: list[str] = [c.name for c in d]
    resulting_points = 0
    ...
    first_card = pop_first_card(pile)
    ...
    data_to_send = {
        ...
        "color": first_card[0:3],  # The card name is 3 chars of color + the number/other properties
        "player": 0
    }
    previous_message = data_to_send
    # End of initialisation

    peeps, socks, addresses, list_of_players = connect_clients(sock)
    ...
    while True:
        print(f"Waiting for player {current_player}")
        ...
        if message["stage"] == Stage.GO or message["stage"] == Stage.DEBUG:
            ...
            if "7" in card and modes[0] and "taken" not in message:
                ...
                socks[swapped_player].sendto(
                    json.dumps(ask).encode("utf-8"), addresses[swapped_player]
                )
                ...
                socks[current_player].sendto(
                    json.dumps(js).encode("utf-8"), addresses[current_player]
                )
            ...
            if "0" in card and modes[0] and "taken" not in message:
                ...
                print(f"{hand}, len {len(hand)}, goes to player {list_of_players[next]}")

                socks[list_of_players[next]].sendto(
                    j.encode("utf-8"), addresses[list_of_players[next]]
                )
            ...
            # send_challenge_to_previous_player(pile, all_played, message)
